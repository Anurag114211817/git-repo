    ...
    if process_this_frame:
        ...
        for face_encoding in face_encodings:
            ...
            if matches[best_match_index]:
                ...
                if name != face : 
                    time = datetime.today().strftime(" %I %M %p").replace(' 0', ' ')
                    #system('espeak -v en+f3 -s 145 "I know you! Your name is, "')
                    #system(f'espeak -v hi+f3 {name}')
                    system(f'''espeak -v en+f3 -s 147 "Good 
                          {'morning' if 'AM' in time else 'afternoon' if int(time[1]) < 4 else 'evening'} 
                          {name.split()[0]}! it's, {time}."''')
                    face = name
